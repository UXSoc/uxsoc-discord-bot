...
if (!fs.existsSync(accounts)) {
    try {
        fs.writeFileSync(accounts, JSON.stringify({}))
    } catch (error) {
...
try {
    const data = fs.readFileSync(accounts, 'utf8');
    try {
...
client.on('ready', () => {
    if (ENVIRONMENT == "DEV") console.log("[INFO] Started in Developer Mode")
        ...
        if (reactionEmoji == '🔥') {
            ...
            if (member.roles.cache.has("1294311955401674762") || member.roles.cache.has("1284754382205882388") || member.roles.cache.has("1284754282075390044") || member.roles.cache.has("1292074089715990549")) {
                ...
                if (!dscrd_nick || (dscrd_nick && (dscrd_nick.split(" ").length < 2))) {
                    ...
                    return user.send(`Hey ${user.displayName} 👋, please follow the server profile name format as seen in <#${config.channels.rules}> before joining a project.`).catch(error => {
                ...
                removeReaction(user, message, reactionEmoji);
                return user.send(`Hey ${user.displayName} 👋, please complete the **Git Gud at Git Workshop** to be able to contribute to community projects.`).catch(error => {
                    console.log("could not send dm to user.")
                });
            }
            return;
